...
fn test_get_random_hex() {
    ...
    let result = pqkd_client.get_random_hex(size as u32).unwrap();
    
    assert_eq!(result, random_hex);
}

#[test]
fn test_get_random_hex_for_the_large_size() {
...
fn test_get_random_base64() {
    ...
    let result = pqkd_client.get_random_base64(size as u32).unwrap();
    
    assert_eq!(result, random_base64);
}

#[test]
fn test_get_random_base64_for_the_large_size() {
    let size = MAX_SIZE_FOR_STRING_FORMAT + 1;
...
fn test_get_random_bytes() {
    ...
    let random_bytes = pqkd_client.get_random_bytes(size as u32).unwrap();
    
    assert_eq!(random_bytes, binary.to_vec());
}

#[test]
fn test_get_random_bytes_for_the_large_size() {
